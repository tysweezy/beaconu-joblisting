...
=======
    <link rel="stylesheet" href="http://localhost/joblisting/css/style.css">
	<!-- twitter bootstrap -->
	<link rel="stylesheet" href="http://localhost/joblisting/css/bootstrap.css">
>>>>>>> 25c093bcdad6385d73dcb8a44b94065d42a4b324
  ...
  $form = new Form();
	      ...
	      /* $gump = new GUMP();
            ...
            $gump->validation_rules(array(
                'apply-email'     => 'required|valid_email',
                'cover-letter'    => 'required'
            ));

            $gump->filter_rules(array(
                'apply-email'   => 'trim|sanitize_email',
                'cover-letter'  => 'trim|sanitize_string'
             ));
            ...
            $validation_data = $gump->run($_POST);
             ...
             if (isset($_POST['apply'])) {
                ...
                if(mail($to, $subject, $body, $from)) {
                  echo '<span class="success">Application sent successfully <span class="right"><a href="/"> View Listings >></a></span></span>';
                } else {
    ...
    if($rows) {
    	...
    	foreach ($rows as $row) {
    		$stageHost = 'http://activatesaga.com/beaconu/listings/apply/' . $row['id'];
        $devHost =  'http://localhost:9000/apply/' . $row['id'];
        $liveHost = 'http://beaconu.decipherinc.com/listings/apply/' . $row['id'];
            ...
            $validation_data = $gump->run($_POST);
             ...
             if (isset($_POST['apply'])) {
                ...
                $maxFileSizeMb = $maxFileSize / 1024; // MB

                $allowedExtensions = array("pdf", "doc", "docx");

                $uploadFileName = basename($_FILES['file']['name']);
                $uploadedFileType = substr($uploadFileName, strrpos($uploadFileName, '.') + 1);

                $uploadedFileSize = $_FILES['file']['size'] / 1024;

                if ($uploadedFileSize > $maxFileSize) {
                  echo "Size of file should be less than $maxFileSizeMb MB rn";
                } else {
                  ...
                  if (!$extensionAllowed) {
                    echo "Uploaded file was not a supported file type.";
                  } else {
                    $pathOfFile = $_SERVER['DOCUMENT_ROOT']. "/listings/pdf/$uploadFileName";

                    $tmp_path = $_FILES['file']['tmp_name'];

                    if (is_uploaded_file($tmp_path)) {
                      if (! copy($tmp_path, $pathOfFile)) {
                        echo "error while copying the uploaded file rn";
                      } else {
                        ...
                        $message = Swift_Message::newInstance("Job Application", 25)
                          ->setFrom(array($email => $name))
                          ->setTo(array("amaciel@decipherinc.com" => "Alex Maciel"))
                          ->setBody($messageText)
                          ->attach(Swift_Attachment::fromPath($pathOfFile));

                          if ($mailer->send($message)) {
                            // if message sent -> delete uploaded file from server
                            echo '<span class="success">Application sent successfully <span class="right"><a href="/"> View Listings >></a></span></span>';
                            unlink($pathOfFile);
                          }
          ...
          <h2>Apply for: <span style="color: #aaa;" class="apply-title"><?php echo $row['job_title'] ?> @ <?php echo $row['company'] ?></span></h2>
                ...
                <form action="" method="post" enctype="multipart/form-data">
                  ...
                  <span class="title">Email</span>
                    <input type="email" placeholder="" name="apply-email"/>
                   </label>
                ...
                <textarea name="cover-letter" class="message" cols="100" rows="7" maxlength="4000" value=""></textarea>
                <span class="countdown"></span>
                </label>

                <input type="submit" class="btn" name="apply" value="Send Application"/>
